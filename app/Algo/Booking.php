...
class Booking
{
    ...
    {
        return 'Sorry, no suites are available in the given dates. Please try another date.';
    }
    ...
    public function suite_available(){  
        $room_type_id = $this->room_type->id;
        $room_bookings = DB::select("
            SELECT room_bookings.*
            FROM room_bookings            
            WHERE FIND_IN_SET(:id, room_bookings.room_type_ids) <> 0
        ", ['id' => $room_type_id]);     
        return $this->room_bookings_check($room_bookings);           
    }
